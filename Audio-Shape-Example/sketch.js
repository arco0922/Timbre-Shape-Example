...
function startClassify() {
  modeClassifier = new ModeClassifier('./models/A_special_model2', audioContext , stream, modelLoaded);
}
...
function getMode() {
  modeClassifier.getMode(function(err, res) {
    if (res!=null) {
      select('#result').html('上の音を聴きながら，図形を見てください<br>音に対して図形の変化はしっくりきますか？');     
      background(250);
